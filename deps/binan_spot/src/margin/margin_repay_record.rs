...
///
/// let request = margin::margin_repay_record("BTC").tx_id(2970933056).current(1).size(100);
/// ```
pub struct MarginRepayRecord {
    asset: String,
...
impl From<MarginRepayRecord> for Request {
    fn from(request: MarginRepayRecord) -> Request {
        ...
        Request {
            path: "/sapi/v1/margin/repay".to_owned(),
            method: Method::Get,
            params,
...
mod tests {
    ...
    #[test]
    fn margin_margin_repay_record_convert_to_request_test() {
        let credentials = Credentials::from_hmac(API_KEY.to_owned(), API_SECRET.to_owned());

        let request: Request = MarginRepayRecord::new("BTC")
            .tx_id(2970933056)
            .current(1)
            .size(100)
            .recv_window(5000)
        ...
        assert_eq!(
            ...
            Request {
                ...
                params: vec![
                    ...
                    ("txId".to_owned(), "2970933056".to_string()),
