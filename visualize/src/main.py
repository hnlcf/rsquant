...
if __name__ == '__main__':
    ...
    data = []
    for i in result:
        (entry_id, name, open_price, high_price,
         low_price, close_price, volume, open_time) = i
        item = [open_time, float(open_price), float(high_price), float(
            low_price), float(close_price), float(volume)]
        data.append(item)
    ...
    mpf.plot(df, type='candle', title='ETHUSDT',
